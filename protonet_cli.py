...
def to_per_pixel_targets_semantic(
    ...
    for t in targets:
        h, w = t["masks"].shape[-2:]
        y = torch.full((h, w), ignore_idx, dtype=t["labels"].dtype, device=t["labels"].device)
...
def compute_prototypes(
    ...
    for imgs, targets in tqdm(dataloader, desc="mean", leave=False):
        ...
        sem = to_per_pixel_targets_semantic(targets, decoder.ignore_idx)   # list of [H,W]
...
def predict_fullres(
    ...
    logits_full = img_tiler.stitch(crop_logits, origins, img_sizes)  # list of [C,H,W]
    return logits_full
...
def cmd_compute_prototypes(args: argparse.Namespace) -> None:
    ...
    # compute
    prototypes, mean = compute_prototypes(decoder, train_loader, img_tiler, tgt_tiler)
    ...
    print(f"[OK] saved to {args.save_path}  "
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="protonet_cli",
                                description="ProtoNet prototype computation & predict (no PL).")
    sub = p.add_subparsers(dest="cmd", required=True)

    # compute-prototypes
    pc = sub.add_parser("compute-prototypes", help="Compute dataset-wide prototypes and mean.")
    pc.add_argument("--root-dir", type=str, required=True, help="ANORAK dataset root")
    pc.add_argument("--fold", type=int, default=0)
    pc.add_argument("--img-size", type=int, nargs=2, default=[448, 448], metavar=("H","W"),
    ...
    pc.add_argument("--tile", type=int, default=448, help="Crop size (pixels) at source resolution.")
    pc.add_argument("--stride", type=int, default=448, help="Stride between crops.")
    pc.add_argument("--weighted-blend", action="store_true", help="Use Hann weighting during stitching.")

    # model/encoder
    pc.add_argument("--encoder-name", type=str, default="hf-hub:MahmoodLab/UNI2-h")
    ...
    pc.add_argument("--metric", type=str, default="L2", choices=["L2","cosine"])
    pc.add_argument("--no-center-feats", action="store_true")
    pc.add_argument("--no-normalize-feats", action="store_true")

    pc.add_argument("--device", type=int, nargs="+", default=[0], help="GPU ids (first is used)")
    pc.add_argument("--save-path", type=str, default="prototypes.pt")

    # predict
    pp = sub.add_parser("predict", help="Predict labels at full resolution with tiling.")
    pp.add_argument("--imgs-npy", type=str, required=True, help="Path to numpy array (B,H,W,3).")
    pp.add_argument("--load-path", type=str, required=True, help="Path to prototypes file (.pt).")
    pp.add_argument("--encoder-name", type=str, default="hf-hub:MahmoodLab/UNI2-h")
